...
class FashionMNISTDataModule(ImageDataModule):
    def __init__(self, dataset_fn, batch_size=128, num_workers=2):
        super().__init__(dataset_fn, batch_size, num_workers)
        self.transforms = Compose([ToTensor(), Normalize((0.286), (0.353))])
...
class EMNISTDataModule(ImageDataModule):
    def __init__(self, dataset_fn, split, batch_size=128, num_workers=2):
        ...
        self.split = split
        self.transforms = {
            "balanced": Compose([ToTensor(), Normalize((0.1751), (0.3332))]),
            "digits": Compose([ToTensor(), Normalize((0.1733), (0.3317))]),
            "letters": Compose([ToTensor(), Normalize((0.1722), (0.3309))]),
            "mnist": Compose([ToTensor(), Normalize((0.1733), (0.3317))]),
        }
...
class CIFAR10DataModule(ImageDataModule):
    def __init__(self, dataset_fn, batch_size=128, num_workers=2):
        ...
        self.transforms = Compose(
            [ToTensor(), Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.2435, 0.2616))]
        )
